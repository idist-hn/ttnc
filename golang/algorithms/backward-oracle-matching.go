...
// BackwardOracleMatching tìm kiếm chuỗi con bằng thuật toán Backward Oracle Matching
// Độ phức tạp thời gian: O(n+m)
// Độ phức tạp không gian: O(m)
// Backward Oracle Matching sử dụng oracle automaton để tối ưu hóa tìm kiếm từ phải sang trái
func BackwardOracleMatching(source, target string) (exist bool, index int) {
	exist = false
	index = -1
