...
// TurboBM tìm kiếm chuỗi con bằng thuật toán Turbo BM
...
//
// Turbo BM là cải tiến của Boyer-Moore
// Sử dụng "turbo" shift: tránh so sánh lại các ký tự đã khớp
// Kết hợp với Bad Character và Good Suffix heuristics
func TurboBM(source, target string) (exist bool, index int) {
	exist = false
	index = -1
	...
	// Bắt đầu tìm kiếm với Turbo shift
	i := 0
	turboShift := 0 // Turbo shift: số ký tự đã khớp từ lần trước
	...
	for i <= sourceLen-targetLen {
		...
		for j >= 0 && j >= turboShift && source[i+j] == target[j] {
			j--
		}
