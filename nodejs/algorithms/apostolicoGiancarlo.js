/**
 * ApostolicoGiancarlo - Thuật toán tìm kiếm chuỗi con bằng Apostolico-Giancarlo
 * 
 ...
 * 
 * Apostolico-Giancarlo kết hợp Boyer-Moore với Galil's optimization
 * So sánh từ phải sang trái, tránh so sánh lại các ký tự đã khớp
 * Sử dụng Bad Character và Good Suffix heuristics
 */

class ApostolicoGiancarlo {
    /**
     * Tìm kiếm chuỗi con bằng thuật toán Apostolico-Giancarlo
     * 
    ...
    static search(source, target) {
        ...
        let i = 0;
        let galilPos = 0; // Galil's optimization: vị trí đã so sánh
        ...
        while (i <= sourceLen - targetLen) {
            ...
            while (j >= 0 && j >= galilPos && source[i + j] === target[j]) {
                j--;
            }
...
module.exports = ApostolicoGiancarlo;
